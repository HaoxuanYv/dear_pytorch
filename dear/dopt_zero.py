...
class _DistributedOptimizer(torch.optim.Optimizer):
    ...
    def _prepare_tensor_fusion(self, module_groups):
        ...
        # 
        self._shard_params = [torch.zeros_like(tensor) for tensor in self._shard_buffers] # just copy a shard params for sharding param
        # print(len(self._shard_buffers[0]), len(self._shard_params[0]))
    ...
    def _prepare_shard_params(self, module_groups):
        ...
        # if(rank()==0):
        #     print("****"*5)
        #     print("pad buffer after init")
        #     print(pad_buffer.size())
        # print("-----"*5)
        # print("shard_param after init")
        # print(shard_param)
    ...
    def _register_hooks(self):
        ...
        # print("pad param to pad grad")
        # print(self._pad_buffers)
    ...
    def _push_to_buffer(self, name, tensor):
        ...
        with torch.no_grad():
            pad_buffer = self._pad_buffers[group_idx]
            # 反向传播之前，先初始化pad_buffers，把原来存的param改为grad
            pad_buffer[start_p:end_p].copy_(tensor.view(-1))
    ...
    def _update_one_module(self, module, module_name, group_idx):
        ...
        # print(pad_param, pad_param.size()) 
        for p in self._module_direct_parameters[module_name]:
            # copy grad values from buffers
            name = self._param_names.get(p)
            if name not in self._param_group_idx:
               continue
            group_idx_p, sub_idx_p, start_p, end_p = self._param_group_idx[name]
            ...
            # if rank() == 0:
            #     print(p.data.view(-1), p.data.size(), "100086")
            p.data.view(-1).copy_(pad_param[start_p:end_p])
            # if rank() == 0: 
            #     print(p.data.view(-1), "update")
            p.grad.fill_(0.0)
    ...
    def _adam_shard_param(self):
        for group in self.param_groups:
            ...
            for i, (shard_param, shard_grad, shard_ea, shard_eas) in enumerate(
                    zip(self._shard_params, self._shard_buffers, self._exp_avgs, self._exp_avgs_sq)):
                ...
                if weight_decay != 0:
                    d_shard = d_shard.add(weight_decay, shard_param.data)
                ...
                bias_correction1 = 1 - beta1 ** (self._num_steps+1) 
                bias_correction2 = 1 - beta2 ** (self._num_steps+1) 
                # bias_correction1 = 1 - beta1 ** shard_step
                # bias_correction2 = 1 - beta2 ** shard_step
                step_size = group['lr'] * math.sqrt(bias_correction2) / bias_correction1
                # if(rank() == 0):
                #     print(shard_param)
                shard_param = torch.addcdiv(shard_param, exp_avg, denom, value=-step_size)

                # if rank() == 0:
                #     print("-"*10)
                #     print(shard_param)
                self._shard_params[i] = shard_param
    ...
    def _allgather_one_group_param(self, group_idx):
        ...
        # print(shard_param, shard_param.size())
    ...
    def _bp_barrier(self):
        ...
        # if rank() == 0:
        #     print("before update param pad grad")
        # pad_len = len(self._pad_buffers[0]) // 4
        # for i in range(0, 5):
        #     if(rank() == i):
        #         print(self._pad_buffers[0][rank()*pad_len:(rank()+1)*(pad_len)])
        #         print("______________")
        #         print(self._shard_buffers)
        #         print("over\n")
        self._adam_shard_param()
        # if rank() == 0:
        #     print(self._shard_params, "10010")
